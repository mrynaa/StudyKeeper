...
<body>
    ...
    <?php 
        ...
        $query = 'SELECT date_trunc(\'second\', avg(r.tempo)) as d_media, 
                  ...
                  from (SELECT sum(tempo) as tempo, data, utente.username
                        from sessione, utente
                        where utente.username = $1 and utente.idu = sessione.username
                        group by data, utente.username 
    ...
    <div id="main" class="container-fluid background" style="align-content: center;">  
        ...
        <?php 
            $query = 'SELECT sum(tempo) as tempo, sum(pause) as pause, 
                        floor(avg(valutazione)) as valutazione, data, utente.username
                      from sessione, utente
                      where utente.username = $1 and utente.idu = sessione.username 
                      group by data, utente.username 
                      order by data';
